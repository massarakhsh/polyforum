...
func BuildFile(rule ruler.DataRuler, level int) Dber {
	it := &FileControl{ }
	it.Mode = "all"
...
func (it *FileControl) Execute(rule ruler.DataRuler) {
	if rule.IsShift("gridinit") {
		...
		it.execOpen(rule)
	} else if rule.IsShift("last") {
		it.execLast(rule)
	} else if rule.IsShift("import") {
		it.execImport(rule)
	} else if rule.IsShift("folder") {
		it.execFolder(rule)
	} else if rule.IsShift("upload") {
		it.execUpload(rule)
	} else if rule.IsShift("store") {
		it.execStore(rule)
	} else if rule.IsShift("clear") {
		it.execClear(rule)
	} else if rule.IsShift("rename") {
		it.execRename(rule)
	} else if rule.IsShift("delete") {
		it.execDelete(rule)
	} else if rule.Top() == "seek" {
		it.Mode = ""
		it.execute(rule)
	} else {
		it.execute(rule)
